<template>
  <section class="section has-background-info">
    <div class="container">
        <div class="text">
          <p class="has-text-white-ter">Accepted abstract & Full paper will be invited to give the presentation at ICCWAMTIP 2024</p>
        </div>
        <div>
          <a class="button is-dark" href="https://cmt3.research.microsoft.com/ICCWAMTIP2020">Submit A Paper Now!</a>
          <a class="button is-dark" href="/Users/lixu/waveletlab-uestc/waveletlab-webbuilder/static/doc/Instructions for using the Microsoft CMT system.docx">Instruction for Submission</a>
        </div>
      </div>
  </section>
</template>
<style scoped>
.section {
  margin-top: 2rem;
  padding: 1.5rem;
}
.container {
  display: flex;
  align-items: center;
}
.text {
  font-size: 1.5rem;
  flex:1;
}
</style>
